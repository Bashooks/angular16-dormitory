...
export class SubmitdocumentsComponent implements OnInit {
  ...
  constructor(private rout: Router, private submitdocumentsService: SubmitdocumentsService) {
  ...
  onSubmit(): void {
    ...
    if (this.personId && this.recipientName && this.documentTitle && this.selectedFile) {
      this.submitdocumentsService.uploadDocument(this.personId, this.recipientName, this.documentTitle, this.typefile, this.selectedFile)
        .subscribe(
          response => {
      ...
      alert('กรุณากรอกข้อมูลให้ครบถ้วน/เลือกไฟล์ใหม่');
    }
  }
